/* Phase 10.1: Core Live Sources Renderer
   Canonical OBS Browser Source renderer migrated from Phase 8.
   Keeps phase8 CSS class names for safe visual compatibility during core merge.
*/
(() => {
  'use strict';

  if (window.__PEPS_CORE_LIVE_SOURCES__) return;
  window.__PEPS_CORE_LIVE_SOURCES__ = true;
  ...
  function renderHealthPanel() {
    ...
    const checks = { groups: !!(state.groups && Object.keys(state.groups).length), schedule: realMatches(state).length, standings: standingsEntries(state).some(([, r]) => r.length), knockout: knockout(state).length, latest: !!latestResult(state) };
    box.className = `phase8-card ${checks.groups && checks.schedule ? 'good' : 'warn'}`;
    box.innerHTML = `<div class="phase8-title"><span>Core · OBS Source Health</span><span class="phase8-badge ${checks.groups && checks.schedule ? 'good' : 'warn'}">${SOURCE_VIEWS.length} Sources</span></div><div class="phase8-text">Copy URL ไปใส่ OBS Browser Source ขนาดแนะนำ 1920×1080</div><div class="phase8-grid">${SOURCE_VIEWS.map(([id, label]) => `<div class="phase8-source"><b>${esc(label)}</b><small>${esc(base + '?view=' + id)}</small><div class="phase8-actions"><button class="btn" data-core-source-copy="${esc(base + '?view=' + id)}">Copy URL</button><button class="btn" data-core-source-open="${esc(base + '?view=' + id)}">Open</button></div></div>`).join('')}</div>`;
  }
  ...
  function bind() {
    document.addEventListener('click', (event) => {
      const copy = event.target.closest('[data-core-source-copy]'); if (copy) copyText(copy.dataset.coreSourceCopy);
      const open = event.target.closest('[data-core-source-open]'); if (open) window.open(open.dataset.coreSourceOpen, '_blank');

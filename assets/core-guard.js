...
(() => {
  ...
  const CORE_ASSET_VERSION = 'phase10-23-20260511-1';
  ...
  function loadPhase1023Hotfix() {
    loadAddonAssets('assets/draw-preview-source-sync.css', 'assets/draw-preview-source-sync.js');
  }

  function realTeams(state) {
    return Array.isArray(state.teams) ? state.teams.filter((t) => clean(t) && !isBye(t)) : [];
  }
  function groupTeams(state) {
    const groups = state.groups && typeof state.groups === 'object' ? state.groups : {};
    return Object.values(groups).flat().filter((t) => clean(t) && !isBye(t));
  }
  ...
  function getChecks() {
    ...
    const matches = realMatches(state);
    const done = doneMatches(state);
    return {
      setup: !!(state.event && clean(state.event.name) && Number(state.event.groupCount || 0) > 0),
      teams: realTeams(state).length > 0,
      drawConfirmed: groupTeams(state).length > 0,
      schedule: matches.length > 0,
      scoresComplete: matches.length > 0 && done.length >= matches.length,
      standings: !!(state.standings && typeof state.standings === 'object' && Object.keys(state.standings).length),
  ...
  function syncDrawControlPreview() {
    ...
    const state = readState();
    const style = clean(document.getElementById('drawAnimation')?.value || state.settings?.drawAnimation || 'wheel') || 'wheel';
    const scale = Number(document.getElementById('drawAnimationScale')?.value || state.settings?.drawAnimationScale || 0.72);
    ...
    stage.querySelectorAll('.draw-graphic,.draw-source-card').forEach((graphic) => {
      Array.from(graphic.classList).forEach((name) => {
        if (name.startsWith('draw-style-') || name.startsWith('mode-')) graphic.classList.remove(name);
      });
    ...
    if (state.settings?.drawAnimation !== style || state.settings?.drawAnimationScale !== scale) {
      writeState({ ...state, settings: { ...(state.settings || {}), drawAnimation: style, drawAnimationScale: scale } });
  ...
  function installDrawControlPreviewHotfix() {
    ...
    document.addEventListener('change', (e) => { if (e.target?.id === 'drawAnimation') setTimeout(run, 0); }, true);
    document.addEventListener('input', (e) => { if (e.target?.id === 'drawAnimationScale') setTimeout(run, 0); }, true);
    const stage = document.getElementById('drawStage');
    if (stage) new MutationObserver(() => setTimeout(run, 0)).observe(stage, { childList: true, subtree: true });
  }

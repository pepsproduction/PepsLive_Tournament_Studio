(()=>{
  ...
  function setPill(text,mode){const p=pill();if(!p)return;p.textContent=text;p.dataset.manualSaveMode=mode||'';}
  function markDirty(){dirty=true;setPill('Manual Save: มีการเปลี่ยนแปลง ยังไม่ได้เซฟ','dirty')}
  function markSaved(){dirty=false;setPill('Manual Save: Saved '+nowText(),'saved')}
  function allowManual(ms=1600){allowUntil=Date.now()+ms;}

  Storage.prototype.setItem=function(key,value){
    if(this===localStorage&&key===KEY){
      if(Date.now()>allowUntil){pendingValue=String(value);markDirty();return;}
      pendingValue=null;
  ...
  function addManualSaveButton(){
    ...
    b.addEventListener('click',()=>{
      if(pendingValue){
        ...
        pendingValue=null;
        markSaved();
      }else if(dirty){markSaved();}
      else setPill('Manual Save: ไม่มีรายการค้างเซฟ','saved');
    });
    const first=top.querySelector('#btnSaveProject');
    top.insertBefore(b,first||null);
  }

  document.addEventListener('click',e=>{const t=e.target&&e.target.closest&&e.target.closest(MANUAL_CLICK_SELECTOR);if(t)allowManual(1800);},true);

  function boot(){
    addManualSaveButton();
    setPill('Manual Save: Ready','ready');
    buttonTimer=setInterval(()=>{
      if(document.getElementById('btnManualSaveLocal')){clearInterval(buttonTimer);buttonTimer=null;return;}
      addManualSaveButton();
    },800);
    setTimeout(()=>{if(buttonTimer){clearInterval(buttonTimer);buttonTimer=null;}},8000);
  }
